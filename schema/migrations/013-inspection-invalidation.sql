-- An approved inspection finding can be disproved by a corrected inspector.
-- Keep the old proposal and decision events, but allow a separately evidenced
-- terminal invalidation. The finding no longer occupies its subject or ADO.
CREATE TABLE workflow_item_rebuilt_013 (
...
INSERT INTO workflow_item_rebuilt_013
...
ALTER TABLE workflow_item_rebuilt_013 RENAME TO workflow_item;
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_workflow_item_one_live_per_subject
  ...
  WHERE current_state NOT IN ('closed', 'superseded', 'invalidated');
