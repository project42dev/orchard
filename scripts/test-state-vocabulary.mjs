...
test('every state the machine defines is accepted by the constraint', async () => {
    ...
    assert.equal(LIFECYCLE_STATES.length, 23, 'the machine defines exactly 23 states');
});
...
test('a version 6 database migrates in place: rows carry over, the live-item index survives, invalid states are rejected', async () => {
    ...
    try {
        ...
        assert.ok(index.sql.includes("WHERE current_state NOT IN ('closed', 'superseded', 'invalidated')"), 'the index must keep its partial WHERE clause');
