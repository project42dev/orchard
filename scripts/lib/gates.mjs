...
function renderGateSummary(manifest) {
    ...
    if (attention.length === 0) {
        return [`**All ${manifest.items.length} item${manifest.items.length === 1 ? '' : 's'} passed every review.** Gate 2 decisions are item-specific; approve each item with its per-item command.`, ''];
    }
    ...
    const lines = [
        ...
        `A bare \`approve\` or \`approved\` comment is not a Gate 2 decision. For a mixed issue like this one, use the per-item command for each item you want to approve.`,
        '',
    ];
