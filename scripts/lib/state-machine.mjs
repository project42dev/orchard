...
const exceptionStates = ["denied", "deferred", "changes-requested", "stale-approval", "blocked", "superseded", "invalidated", "externally-published"];
...
export function isTransitionAllowed(transition) {
    ...
    if (!["closed", "denied", "superseded", "invalidated", "externally-published"].includes(from) && blockCauses.has(cause) && to === "blocked") return true;
    if (!["closed", "superseded", "invalidated", "externally-published"].includes(from) && cause === "superseded" && to === "superseded" && transition.superseding_item_id) return true;
    if (from === "ado-linked" && to === "invalidated" && cause === "inspection-invalidated") return true;
    if (["ado-linked", "executing", "gate2-ready", "gate2-pending", "gate2-approved", "blocked", "changes-requested", "stale-approval"].includes(from)
        && to === "externally-published" && cause === "external-publication-reconciled") return true;
