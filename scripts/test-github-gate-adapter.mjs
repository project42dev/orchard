...
test('a bare approve comment is refused because it names no item', async () => {
    const { itemId, body } = await fixture();
    await assert.rejects(
        () => fetchVerifiedEvent({ ...reference, expected_item_id: itemId }, {
...
test('bare approve is refused regardless of casing or surrounding whitespace', async () => {
    const { itemId, body } = await fixture();
...
test('a bare deny comment is refused because it names no item', async () => {
    const { itemId, body } = await fixture();
    await assert.rejects(
        () => fetchVerifiedEvent({ ...reference, expected_item_id: itemId }, {
...
test('an edited bare approve comment is still refused because it names no item', async () => {
    const { itemId, body } = await fixture();
    await assert.rejects(
        () => fetchVerifiedEvent({ ...reference, expected_item_id: itemId }, {
            env: ENV,
