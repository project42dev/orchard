...
test('a Gate 2 issue where every item passed review still requires the per-item approve commands', () => {
  ...
  assert.ok(body.includes('Use the per-item command shown below to approve each one.'), 'even a clean Gate 2 batch must require explicit per-item approval');
...
test('a Gate 2 issue with one failed review warns against bare-approve and names exactly which item, and why -- found live 2026-08-17', () => {
  ...
  assert.ok(body.includes('Whole-issue approval does not change state here'),
    'the summary must explicitly say a whole-issue approval is ignored, not leave the owner guessing');
