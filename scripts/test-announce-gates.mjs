...
test('a Gate 2 issue where every item passed review still requires per-item approval commands', () => {
  ...
  assert.ok(body.includes('Gate 2 decisions are item-specific'), 'the summary must require per-item decisions even when all items are clean');
  assert.ok(!body.includes('approves all of them'), 'the summary must not claim bare whole-issue approval');
...
test('a Gate 2 issue with one failed review says bare comments do not decide and names exactly which item, and why -- found live 2026-08-17', () => {
  ...
  assert.ok(body.includes('A bare `approve` or `approved` comment is not a Gate 2 decision'),
    'the summary must explicitly say bare whole-issue comments are refused');
