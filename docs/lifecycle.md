...
**Reconciled 2026-09-06.** This page is the merge of two documents that
described the same subject and had drifted apart: Orchard's own state-machine
reference, and the narrative lifecycle page that was extracted from the public
platform repository when Orchard's internals were moved back behind their own
boundary. There is now one account.

**Related:** ADR-0017 (the separation), ADR-0018 (the model map), ADR-0019 (the
content database), ADR-0022 (two tracks, one lifecycle), ADR-0023 (workflow
state).

> **Historical note, 2026-08-15, superseded.** Earlier versions of this page
> carried a banner reading "Orchard is not running anywhere", written after the
> 2026-08-14 reset to a never-run state (commit `0f05642`). That was true when
> written and was **superseded by the production bring-up of 2026-08-16 to
> 2026-08-20**. It is recorded here because it is the record of a real reset,
> not because it describes today. [Status](status.md) carries the evidence for
> both halves of it, kept in separate columns.

> **The states below are the states the machine actually enforces.** They were
...
```text
                 [ TRACK 1 DISCOVERY / TRACK 2 INSPECTION ]
                                    |
                                    v
                             +--------------+
                             |   observed   |
                             ...
                             +------+-------+
                                    ...
                                    v
                         +--------------------+
                         |   gate1-pending    |  <-- the spend and editorial control
                         +---------+----------+
              +--------------------+--------------------+------------------+
              | approved           | denied             | deferred         | changes
              v                    v                    v                  v
     +----------------+    +------------+        +------------+    +--------------------+
     | gate1-approved |    |   denied   |        |  deferred  |    | changes-requested  |
     +--------+-------+    +------------+        +------------+    +--------------------+
              | ado-reconciled
              v
     +----------------+
     |   ado-linked   |  work item created in the tracker (AB#...)
     +--------+-------+
              | execution-started
              v
     +----------------+
     |   executing    |  researcher > drafter > verifier > adversary > arbiter > finalizer
     +--------+-------+
              | artifact-ready
              v
     +----------------+
     ...
     +--------+-------+
              ...
              v
     +----------------+
     | gate2-pending  |  manifest and diff announced, bound to the artifact digest
     +--------+-------+
              | decision-approved
              v
     +----------------+
     ...
     +--------+-------+
              ...
              v
  +-----------------------+   validation-passed   +------------------------+
  | publication-preparing | --------------------> | publication-validating |
  +-----------------------+                       +-----------+------------+
                                                              | pr-reconciled
                                                              v
  +-----------------------+   merge-started       +------------------------+
  |  publication-merging  | <-------------------- |  publication-pr-open   |
  +-----------+-----------+                       +------------------------+
              | publication-acknowledged
              v
     +----------------+
     |   published    |  exact protected-main acknowledgement recorded
     +--------+-------+
              | closure-evidence-ready
              v
  +--------------------+
  | ado-closure-ready  |  closure packet assembled; the owner must accept it
  +---------+----------+
            | closure-accepted
            v
     +----------------+
...
| State | Meaning and trigger | Evidence produced |
|---|---|---|
| `observed` | Recorded by a Track 1 survey or a Track 2 inspection. | Row in `workflow_item`, observation in `observation_event`. |
| `proposed` | A candidate document exists and is ready to be gated. | `item_revision` carrying a proposal digest. |
| `gate1-pending` | Batched and announced on the Gate 1 issue. | Issue with the manifest table and the command grammar. |
| `gate1-approved` | Authorized by an owner comment naming the exact item and revision. | `decision_event` bound to an authenticated provider event. |
| `ado-linked` | Tracker work item created and reconciled. | `external_link` row. Created on approval, never before. |
| `executing` | Claimed by the authoring ensemble. | Persisted role handoffs. |
| `gate2-ready` | Artifact drafted, evidence compiled. | `artifact_binding` bound to the item revision. |
| `gate2-pending` | Gate 2 manifest announced with the full artifact and its digest. | Issue carrying the artifact itself, not a pointer to it. |
| `gate2-approved` | The owner authorized that exact artifact digest. | `decision_event`; invalidated if the artifact changes. |
| `publication-preparing` | The branch and commit tree are being built, including the registry entry. | Deterministic branch. |
| `publication-validating` | The prepared tree is checked against the approved digest. | Validation record. |
| `publication-pr-open` | Pull request opened against protected `main`. | PR head, base, tree and diff digests. |
| `publication-merging` | Merge in flight. | `publication_transaction` row. |
| `published` | Merge acknowledged against protected main by exact reconciliation. | Merge commit, tree and diff all matched. |
| `ado-closure-ready` | Closure packet assembled, awaiting owner acceptance. | `closure_packet` row. |
| `closed` | The owner accepted the packet; the tracker item may close. | Owner event bound to the packet digest. |

### Exception states

These are reachable only through the exception rules in `isTransitionAllowed`,
never through the direct transition table.

| State | Reached from | On |
|---|---|---|
| `denied` | either pending gate | `decision-denied`. A new revision recovers it: Gate 1 back to `proposed`, Gate 2 back to `executing`. |
...
machine holds. Its result is evidence in the closure packet.

**`workflow_item.current_state` is CHECK-constrained** as of migration
`007-workflow-item-state-check.sql`, so a typo can no longer persist as
authoritative state. That was a real defect and it is closed.
...
**Built, 2026-09-06.** A third intake path alongside discovery and the
...
- **Batch approval is not designed.** Every decision command is per item and
  ...
  typing them.
- **All six agent roles are wired (closed 2026-08-20).** Researcher and
  finalizer were the two that were missing. The gap was in the brief generator,
  not in the delivery platform, and both now appear in `ROLE_JOBS` and
...
- **Request intake is built (closed 2026-09-06).** Corrected: this bullet said
  ...
  `.github/workflows/curriculum-request-ingest.yml` runs it, and requests join
  the same discovery queue and the same Gate 1 a discovered candidate uses.
  Landed across `f27c318`, `25796b0` and `3539d89`. A request carries its own
  `pathId`, so unlike a discovery probe it has always resolved to a real
  learning path.
...
a config/diagrams.json file that exists in no repository, and the retired
