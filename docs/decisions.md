...
> This banner read "Orchard is not running anywhere". It described the
> 2026-08-14 reset to a never-run state (commit `0f05642`): the work queue and
> the candidate list emptied, every artifact of the previous runs removed, ahead
> of the first full start-to-finish test. **That test happened.** Production
> bring-up ran 2026-08-16 to 2026-08-20 and Orchard has been running since. The
> banner is kept, marked, rather than deleted, because it is the record of a
> real reset. See [Status](status.md) for what is built, what is deployed, and
> what has actually been proven, kept in separate columns — including the two
> capabilities that are still not green.

The full architecture decision records live in the private planning repository
`project42dev-ops`, by deliberate decision recorded in
[`REPO-BOUNDARY.md`](../REPO-BOUNDARY.md): candid early analysis needs somewhere
it can be candid, and this repository is public, so it carries a **summary of
the accepted decisions a contributor or an adopter needs**. This page is that
summary in narrative form; [`adr/`](adr/index.md) is the same material one
decision per page. Nothing on either links to a repository you cannot open.
