...
rejected, live in the private planning repository `project42dev-ops`. That
split is itself a decision, recorded in [`REPO-BOUNDARY.md`](../../REPO-BOUNDARY.md):
analysis is more honest when it has somewhere private to be honest, and what a
contributor or an adopter needs is the accepted decision and the reason it
holds. These pages are that, written to stand alone. **Nothing here links to
anything you cannot open.**

**The numbering has gaps, and they are not missing pages.** This is one
continuous Orchard series that predates the move of these records into this
repository; 0015 and 0017 through 0028 are the accepted decisions that shape
Orchard. The earlier numbers, and 0016, belong to decisions about other parts of
the estate and are not Orchard's to publish.
...
These pages deliberately omit private topology, identity and secret names,
operational schedules and cost figures, because those describe one
organisation's deployment rather than the portable capability. **That concrete
deployment is documented separately in this same repository**, in
[Architecture](../architecture.md) and the [Operations runbook](../operations.md).
If you came here looking for resource names or schedules, those are the two
pages you want.
