...
- **Whole Issue Approval**: Gate 2 does not support whole-issue `approve`/`approved`/`deny`/`denied` comments. Decisions are item-specific only.
- **Specific Item Approval**:
  ```text
...
- **Several Items in One Comment**: the GitHub Actions reviewer
  ...
  in-container path (`apply-gate-decisions.mjs`) still reads one named item per
  comment, and likewise refuses bare whole-issue Gate 2 comments.
